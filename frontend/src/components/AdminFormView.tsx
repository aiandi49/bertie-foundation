  ...
  const fetchSubmissions = async () => {
    ...
    try {
      if (formType === 'newsletter') {
        ...
        // For non-newsletter forms, use FormServiceFallback (localStorage-based)
        const collectionName = COLLECTION_MAPPING[formType];
        console.log(`Fetching ${formType} submissions from localStorage: ${collectionName}`);

        const localData = FormServiceFallback.getStoredSubmissions(collectionName);
        ...
        <div className="space-y-3 sm:space-y-4 max-h-[650px] overflow-y-auto pr-1 sm:pr-2 mt-3 sm:mt-4">
          {formType === 'newsletter' ? (
            <table className="min-w-full divide-y divide-gray-700">
              <thead>
                <tr>
                  ...
                  <th className="p-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Name</th>
              ...
              <tbody className="divide-y divide-gray-700">
                {submissions
                  .filter(submission => {
                    ...
                    return (submission.status || 'active') === filterStatus;
                  })
                  .map(submission => (
                    <tr
                      key={submission.id}
                      className="hover:bg-secondary-700/80 transition-colors group"
                    >
                      <td className="p-3 whitespace-nowrap text-sm text-gray-200">{submission.email}</td>
                      <td className="p-3 whitespace-nowrap text-sm text-gray-200">{submission.name || 'N/A'}</td>
                      <td className="p-3 whitespace-nowrap text-sm">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          (submission.status || 'active') === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}>
                          {submission.status || 'active'}
                        </span>
                      </td>
                      <td className="p-3 whitespace-nowrap text-sm text-gray-400">
                        {getFormattedDate(submission.subscribed_at || submission.submittedAt)}
                      </td>
            ...
            submissions.map(submission => (
              ...
              >
                <div className="text-xs text-gray-400 mb-2">{getFormattedDate(submission.submittedAt)}</div>
