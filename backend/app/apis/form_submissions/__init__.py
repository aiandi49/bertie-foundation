...
def create_submission(submission: FormSubmissionCreate) -> FormSubmissionResponse:
    ...
    if supabase_available():
        try:
            get_supabase().table("form_submissions").insert({
                "id": submission_id,
    ...
    return FormSubmissionResponse(success=True, message="Form submission received", submission_id=submission_id)


@router.get("/", response_model=FormSubmissionsList)
def get_all_submissions(
    form_types: Optional[List[FormType]] = Query(None),
    search: Optional[str] = Query(None),
) -> FormSubmissionsList:
    if not supabase_available():
        return FormSubmissionsList(submissions=[])
    try:
        supabase = get_supabase()
        result = supabase.table("form_submissions").select("*").order("timestamp", desc=True).execute()
...
def delete_submission(submission_id: str) -> FormSubmissionResponse:
    if not supabase_available():
        raise HTTPException(status_code=503, detail="Database not configured")
    try:
        get_supabase().table("form_submissions").delete().eq("id", submission_id).execute()
